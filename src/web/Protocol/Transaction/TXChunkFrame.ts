...
export class TXChunkFrame {
    public static Deserialize(value: CAMPBuffer): TXChunkMessage {
    ...
    public static Serialize(sid: bigint, txId: number, offset: bigint, payload: CAMPBuffer): CAMPBuffer {
        if (payload.byteLength > CAMP_MAX_PAYLOAD)
            throw new SerializationError(`Payload size of ${CAMP_MAX_PAYLOAD} bytes exceeded!`);

        const msg_buf = CAMPBuffer.alloc(8 + 1 + 4 + 8 + payload.byteLength);
